...
from amunty.services.llm_core import llm_call
...
def _process_text_file(path: str) -> str:
    ...
    try:
        from amunty.services.personal_docs import read_text_file
...
def _load_vl_settings() -> dict:
    ...
    try:
        from amunty.services.settings import load_settings
...
def _resolve_vl_model(configured: str) -> tuple:
    ...
    """
    from amunty.services.ai_interaction import _resolve_model
...
def analyze_image_with_vl_result(image_path: str) -> dict:
    ...
    try:
        ...
        try:
            from amunty.services.endpoint_resolver import resolve_vision_fallback_candidates
    ...
    for fid in attachment_ids:
        ...
        if upload_handler.is_image_file(path, mime):
            ...
            if mime == "application/pdf":
                ...
                if session_id:
                    try:
                        from amunty.services.pdf_forms import has_form_fields, extract_fields
                        from amunty.services.pdf_form_doc import (
                            save_field_sidecar,
                        ...
                        if doc_id and auto_opened_docs is not None:
                            from amunty.services.database import SessionLocal, Document
                            _db = SessionLocal()
                            try:
