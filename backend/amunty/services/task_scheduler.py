...
def _resolve_task_timezone(db, task) -> str | None:
    ...
    if not getattr(task, "crew_member_id", None):
        return None
    try:
        from amunty.models.odysseus_models import CrewMember
...
class TaskScheduler:
    ...
    async def start(self):
        ...
        try:
            from amunty.models.odysseus_models import SessionLocal, TaskRun
            db = SessionLocal()
            try:
        ...
        try:
            from amunty.models.odysseus_models import SessionLocal, CrewMember, ScheduledTask
            db = SessionLocal()
            try:
        ...
        try:
            from amunty.models.odysseus_models import SessionLocal, ScheduledTask
            db = SessionLocal()
            try:
    ...
    async def _note_pings_loop(self):
        ...
        from amunty.services.builtin_actions import action_ping_notes, TaskNoop
        while self._running:
    ...
    async def _event_pings_loop(self):
        ...
        from amunty.services.builtin_actions import action_ping_events, TaskNoop
        while self._running:
            owners = self._known_task_owners()
            for ow in (owners or [""]):
                try:
    ...
    def _known_task_owners(self) -> list:
        ...
        """
        from amunty.models.odysseus_models import SessionLocal, ScheduledTask, Note
        db = SessionLocal()
        try:
            owners = set()
    ...
    async def _loop(self):
        ...
        while self._running:
            ...
            sleep_for = 60.0
            try:
                from amunty.models.odysseus_models import SessionLocal as _SL, ScheduledTask as _ST
                _db = _SL()
                try:
    ...
    async def _execute_task_locked(self, task_id: str, run_id: str, *, release_executing: bool = True):
        from amunty.models.odysseus_models import SessionLocal, ScheduledTask, TaskRun
        ...
        try:
            ...
            from amunty.services.builtin_actions import TaskDeferred, TaskNoop
    ...
    def _log_to_assistant(self, db, task, result_text: str):
        ...
        if (task.action or "") in self._SILENT_ACTIONS:
            return
        from amunty.services.assistant_log import log_to_assistant
        log_to_assistant(
            task.owner,
            result_text[:1000],
    ...
    async def _execute_action(self, task) -> tuple:
        ...
        from amunty.services.builtin_actions import BUILTIN_ACTIONS

        action_fn = BUILTIN_ACTIONS.get(task.action)
        if not action_fn:
            return f"Unknown action: {task.action}", False

        from amunty.services.builtin_actions import TaskNoop
        try:
    ...
    async def _execute_checkin(self, task, crew, db, session_id: str,
        ...
        from amunty.services.tool_implementations import do_manage_notes
        from amunty.services.agent_tools import get_mcp_manager
        ...
        try:
            from amunty.models.odysseus_models import SessionLocal as _SL, CalendarEvent as _CE
            _db = _SL()
            try:
    ...
    async def _execute_llm_task(self, task, db) -> str:
        ...
        from amunty.models import Conversation as DbSession, ChatMessage, CrewMember
        ...
        if crew and crew.enabled_tools:
            try:
                ...
                if isinstance(enabled, list) and enabled:
                    from amunty.services.tool_index import BUILTIN_TOOL_DESCRIPTIONS
        ...
        relevant_tools = None
        try:
            from amunty.services.tool_index import get_tool_index, ASSISTANT_ALWAYS_AVAILABLE
        ...
        try:
            ...
            from amunty.services.llm_core import llm_call_async
            messages = [
        ...
        try:
            from amunty.services.text_helpers import strip_think
    ...
    async def _deliver_task_result(self, task, result: str, db, model: str = None):
        ...
        if self._session_manager:
            try:
                from amunty.models import Message as MemMsg
    ...
    async def _run_agent_loop(self, endpoint_url: str, model: str, task, session_id: str,
        ...
        from amunty.services.agent_loop import stream_agent_loop
        ...
        headers = {}
        try:
            from amunty.models.odysseus_models import SessionLocal, ModelEndpoint
            from amunty.services.endpoint_resolver import normalize_base, build_headers
            db2 = SessionLocal()
            try:
        ...
        try:
            from amunty.services.endpoint_resolver import resolve_utility_fallback_candidates
        ...
        if not full_text.strip():
            try:
                from amunty.services.llm_core import llm_call_async_with_fallback
                from amunty.services.endpoint_resolver import resolve_utility_fallback_candidates
    ...
    async def _execute_research_task(self, task, db) -> str:
        ...
        from amunty.models import Conversation as DbSession, ChatMessage
        from amunty.services.deep_research import DeepResearcher
        from amunty.services.research_handler import RESEARCH_DATA_DIR, ResearchHandler
        from amunty.services.research_utils import strip_thinking
        from amunty.services.settings import get_setting
        ...
        if not endpoint_url or not model:
            try:
                from amunty.services.endpoint_resolver import resolve_endpoint
        ...
        try:
            ...
            try:
                from amunty.services.event_bus import fire_event
    ...
    def _has_chain_cycle(self, db, start_id: str, max_depth: int = 10) -> bool:
        ...
        from amunty.models.odysseus_models import ScheduledTask
        visited = set()
        current = start_id
    ...
    def _resolve_defaults(self, db, owner):
        ...
        from amunty.models import Conversation as DbSession
        try:
